...
namespace wpr_command_center {
	...
	*****************************************************************************/
		...
		, qnode(argc,argv)
	{
		...
		QObject::connect(map_view, SIGNAL(signalSetRobotMove(int,float,float,float)), &qnode, SLOT(slotSetRobotMove(int,float,float,float)));
		...
		QObject::connect(&qnode, SIGNAL(signalUpdateFollowerPose(int, float, float, float, float)), map_view, SLOT(slotUpdateFollowerPose(int, float, float, float, float)));
	...
	*/
	...
	void MainWindow::on_button_robot_move_clicked(bool check ) 
	{
		if(bStopCmd == false)
			map_view->SetMode(MAP_MODE_ROBOT_MOVE);
	}
