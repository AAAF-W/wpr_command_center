...
void QNode::init_ros_comm()
{
	...
	robot_move_pub =  n.advertise<wpr_warehousing_monitor::RobotPose>("/wpr_warehousing/ext_robot_move", 10);
	...
	follower_pose_sub = n.subscribe<geometry_msgs::PoseArray>("/wpb_follower_pose",10,&QNode::FollowerPoseCallback,this);
...
void QNode::FollowerPoseCallback(const geometry_msgs::PoseArray::ConstPtr& msg)
{
	int nFollowerNum = msg->poses.size();
	float leader_x,leader_y;
	if(nFollowerNum > 0)
	{
		leader_x = msg->poses[0].position.x;
		leader_y = msg->poses[0].position.y;
	}
	for(int i=1;i<nFollowerNum;i++)
	{
		float follower_x = msg->poses[i].position.x;
		float follower_y = msg->poses[i].position.y;
		emit signalUpdateFollowerPose(i , leader_x, leader_y, follower_x, follower_y);
	}
}
...
void QNode::slotSetRobotMove(int inID, float inX, float inY, float inYaw)
{
	wpr_warehousing_monitor::RobotPose msg;
	msg.robot_id = inID;
	msg.x = inX;
