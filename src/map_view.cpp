...
CMapView::CMapView(QWidget *parent)
{
    ...
    scene->addItem(&arFollowerItem[0]);
    scene->addItem(&arFollowerItem[1]);
...
void CMapView::Initial()
{
    ...
    robot_client[0].SetFootprintName("robot_1/base_footprint");
    robot_client[1].SetID(2);
    robot_client[1].SetFootprintName("robot_2/base_footprint");
    robot_client[2].SetID(3);
    robot_client[2].SetFootprintName("robot_3/base_footprint");
...
void CMapView::SetMode(int inMode)
{
    ...
    switch(nMode)
    {
        ...
        case MAP_MODE_DRAW_PATH:
            ...
            break;   
        case MAP_MODE_ROBOT_MOVE:
            arrow_item.penArrow = QPen(Qt::blue, 3, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin);
            labelInfo->setText("直线移动模式");
            break;
        default:
...
void CMapView::slotReadMap()
{
    ...
    ROS_WARN("[CMapView] new map  w= %d  h=%d ",width,height);
...
void CMapView::mousePressEvent(QMouseEvent *event)
{
    ...
    {
        bLeftPressed = true;
        if(nMode  == MAP_MODE_NAVI_POSE || nMode == MAP_MODE_ROBOT_POSE || nMode == MAP_MODE_ROBOT_MOVE)
        {
            // 判断是否有机器人被选中
            bRobotSelected = false;
            for(int i=0;i<ROBOT_NUM;i++)
...
void CMapView::mouseReleaseEvent(QMouseEvent *event)
{
    if(event->button() == Qt::LeftButton)
    {
        ...
        if(nMode == MAP_MODE_ROBOT_MOVE)
        {
            ...
            float pose_angle = CalArrowAngle(pntPressed,pntCurrent);
            //ROS_WARN("pose_angle = %.2f",pose_angle);
            emit signalSetRobotMove(arRobotItem[nSelectedRobotIndex].m_nID, pntPressed.x()*-0.01, pntPressed.y()*0.01,pose_angle );
            // 恢复到普通模式
            bRobotSelected = false;
            nMode = MAP_MODE_NORMAL;
            labelInfo->setText("");
...
void  CMapView::slotUpdateFollowerPose(int inIndex, float inLeaderX, float inLeaderY, float inFollowerX, float inFollowerY)
{
    //ROS_WARN("slotUpdateFollowerPose( %d ) (%.2f , %.2f)  (%.2f , %.2f)",inIndex,  inLeaderX,  inLeaderY,  inFollowerX,  inFollowerY);
    if(inIndex >= 1 && inIndex <=2)
    {
        arFollowerItem[inIndex-1].pntSrc = QPoint(-inLeaderX*100,inLeaderY*100);
        arFollowerItem[inIndex-1].pntDest = QPoint(-inFollowerX*100,inFollowerY*100);
        arFollowerItem[inIndex-1].show();
    }
    scene->update();
}